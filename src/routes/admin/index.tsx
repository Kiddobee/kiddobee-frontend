...
async function fetchDashboard() {
  ...
  const approved = bs.filter((b: any) => {
    ...
    return s === "verified" || s.includes("approved");
  }).length;
  ...
  const pending = (interviews.data ?? []).filter((i: any) => {
    ...
    return s.includes("pending") || s.includes("scheduled");
  }).length;
  return { babysitters: babysitters.count ?? bs.length, approved, parents: parents.count ?? 0, openRequests: openReqs, upcoming, pending, alerts: alerts.data ?? [] };
}
